...
for c in COND_KEYS:
    ...
    for r in for_analysis:
        ...
        wc = (
            len(" ".join(shown).split())
            if isinstance(shown, list)
            else len(shown.split())
        )
        wcs.append(wc)
    wc_data[c] = wcs
...
for item in SURVEY_ITEMS:
    ...
    survey_scores[item]["kruskal_wallis"] = {
        "H": round(H, 3),
        "p": f"{p:.2e}" if p < 0.001 else f"{p:.4f}",
    }
...
for item in SURVEY_ITEMS:
    ...
    for c in COND_KEYS:
        vals = [s[item] for s in survey_data[c] if item in s]
        print(
            f"    {c}: M={np.mean(vals):.2f}, SD={np.std(vals, ddof=1):.2f}, n={len(vals)}"
        )
    for a, b in [
        ("single", "integrated"),
        ("single", "competing"),
        ...
        try:
            U, p_val = sp_stats.mannwhitneyu(
                va, vb, alternative="two-sided", method="asymptotic"
            )
...
for c in COND_KEYS:
    ...
    for r in for_analysis:
        ...
        if (
            "CAUSE" in text.upper()
            or "PREDICTION" in text.upper()
            or "cause" in text.lower()
        ):
            compliant += 1
    format_compliance[c] = {
        "compliant": compliant,
        "total": total,
        "pct": round(100 * compliant / total, 1),
    }
...
for c in COND_KEYS:
    ...
    response_word_counts[c] = {
        "mean": round(float(np.mean(wcs)), 2),
        "sd": round(float(np.std(wcs, ddof=1)), 2),
    }
...
within_sample_sizes = {}
for c in COND_KEYS:
    within_sample_sizes[c] = len(
        [r for r in within_for_analysis if r["condition"] == c]
    )
...
for item in SURVEY_ITEMS:
    ...
    try:
        ...
        H, p = float("nan"), float("nan")
    within_survey_scores[item]["kruskal_wallis"] = {
        "H": round(H, 3),
        "p": f"{p:.2e}" if p < 0.001 else f"{p:.4f}",
    }
...
H, p = sp_stats.kruskal(
    *[
        [
            len(" ".join(r["result"]["shown"]).split())
            if isinstance(r["result"]["shown"], list)
            else len(r["result"]["shown"].split())
            for r in within_for_analysis
            if r["condition"] == c
        ]
        for c in COND_KEYS
...
for c in ["integrated", "competing"]:
    ...
    within_divergence[c] = {
        "pass": passed,
        "fail": len(recs) - passed,
        "null": 0,
        "total": len(recs),
        "pass_rate_pct": round(100 * passed / len(recs), 1),
    }
...
within_token_usage["overall"] = {
    "total_input_tokens": ti,
    "total_output_tokens": to,
    "total_cost_usd": round(tc, 4),
    "mean_cost_per_run_usd": round(tc / len(within_for_analysis), 4),
}

within_response_wc = {}
for c in COND_KEYS:
    wcs = [
        len(r.get("simulated_response", "").split())
        for r in within_for_analysis
        if r["condition"] == c and r.get("simulated_response")
    ]
    within_response_wc[c] = {
        "mean": round(float(np.mean(wcs)), 2),
        "sd": round(float(np.std(wcs, ddof=1)), 2),
    }
...
within_reliability = {
    "total_runs": len(within_records),
    "successful_runs": len(within_for_analysis),
    "error_runs": len(within_records) - len(within_for_analysis),
    "success_rate_pct": round(100 * len(within_for_analysis) / len(within_records), 1),
}
...
for item in ["perceived_disagreement", "perceived_breadth", "cognitive_load"]:
    ...
    for a, b in [
        ...
        pairwise[item][f"{a}_vs_{b}"] = {
            "U": U,
            "p": f"{p_val:.4e}",
            "r_rank_biserial": round(r_rb, 3),
            "n1": len(va),
            "n2": len(vb),
            "mean1": round(float(np.mean(va)), 2),
            "mean2": round(float(np.mean(vb)), 2),
        }
...
def code_response_heuristic(text: str) -> dict:
    ...
    specific_refs = len(
        re.findall(
            r"\b(customer|supplier|competitor|market|price|cost|quality|demand|supply|power|transport|capital|loan|credit|stock|inventory)\b",
            text_lower,
        )
    )
    ...
    eval_phrases = len(
        re.findall(
            r"(on the other hand|at the same time|more important|less important|the real|the main|the biggest|actually|in fact|really)",
            text_lower,
        )
    )
    ...
    assumption_words = len(
        re.findall(
            r"(assum|if |what if|suppose|maybe|perhaps|uncertain|not sure|don\'t know|question is|wonder)",
            text_lower,
        )
    )
    ...
    evidence_words = len(
        re.findall(
            r"(test|try|experiment|check|verify|find out|look into|investigate|survey|ask|measure|track|monitor|data|evidence|prove|sign|indicator)",
            text_lower,
        )
    )
    ...
    action_words = len(
        re.findall(
            r"(will|plan|going to|start|begin|need to|should|must|want to|intend|next step|first thing|my plan|i will|i need|i should|i want|i am going|let me)",
            text_lower,
        )
    )
    linking = len(
        re.findall(
            r"(so that|in order to|because of this|therefore|that\'s why|this means|which means|this will help|to address|to solve|to fix)",
            text_lower,
        )
    )
    ...
    cause_markers = re.findall(
        r"(another|also|second|third|additionally|plus|besides|on top of|as well|and also|not only)",
        text_lower,
    )
    ...
    novel_words = len(
        re.findall(
            r"(never thought|didn\'t realize|new idea|different|fresh|perspective|angle|approach|rethink|reconsider|change|shift|pivot|innovative|creative|unique)",
            text_lower,
        )
    )
