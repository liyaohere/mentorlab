...
async def main():
    ...
    parser.add_argument(
        "--concurrency", type=int, default=2, help="Max concurrent API calls"
    )
    ...
    if len(profiles) < 120:
        print(
            f"ERROR: Expected 120 profiles, got {len(profiles)}. Run generate_profiles_supplement.py first."
        )
        sys.exit(1)
    ...
    if args.test in ("within", "both"):
        ...
        errors = sum(
            1
            for r in results
            if isinstance(r, Exception) or (isinstance(r, dict) and r.get("errors"))
        )
    ...
    print(
        f"Errors: {sum(1 for r in all_results if isinstance(r, dict) and r.get('errors'))}"
    )
