...
async def test_transcribe_audio(client: AsyncClient, seed_invite_codes, mock_claude):
    ...
    with patch("app.services.whisper_service.WhisperService.transcribe", fake_transcribe):
        # Create a small fake audio file
        audio_data = b"\x00" * 1024  # 1KB of zeros (not real audio, but Whisper is mocked)
...
@pytest.mark.asyncio
async def test_transcribe_rejects_invalid_type(client: AsyncClient, seed_invite_codes, mock_claude):
    token = await _get_token(client)
