...
class Conversation(Base):
    ...
    __table_args__ = (Index("idx_conversations_participant", "participant_id"),)
    ...
    initiated_by: Mapped[InitiatorType] = mapped_column(
        Enum(InitiatorType), nullable=False
    )
    ...
    # Diagnosis phase
    orchestrator_output: Mapped[dict | None] = mapped_column(
        JSON
    )  # 3 causal directions
    diagnosis_raw: Mapped[list | None] = mapped_column(
        JSON
    )  # All agent outputs (always stored)
    diagnosis_integrated: Mapped[str | None] = mapped_column(
        Text
    )  # C2 only: integrator output
    diagnosis_shown: Mapped[str | None] = mapped_column(
        Text
    )  # What entrepreneur actually saw
